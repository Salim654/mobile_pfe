...
class _PurchaseOrdersPageState extends State<PurchaseOrdersPage> {
  ...
  void fetchInvoices() async {
    try {
      List<Facture>? userInvoices = await ServiceFacture.getPOs();
      setState(() {
        purshases = userInvoices ?? [];
        isLoading = false;
      });
    } catch (e) {
      setState(() {
  ...
  Widget buildListView() {
    ...
    } else if (purshases.isEmpty) {
      return Center(
        child: Image.asset('assets/images/noPO.PNG'),
      );
    } else {
      return ListView.builder(
        itemCount: purshases.length,
        itemBuilder: (context, index) {
          return buildFactureCard(context, purshases[index]);
        },
      );
    }
  }
