...
class _InvoicesPageState extends State<InvoicesPage> {
  ...
  void fetchInvoices() async {
    try {
      List<Facture>? userInvoices = await ServiceFacture.getInvoices();
      setState(() {
  ...
  Widget buildListView() {
    if (isLoading) {
      ...
      return Center(
        child: Image.asset('assets/images/noinvoice.PNG'),
      );
    } else {
      return ListView.builder(
        itemCount: invoices.length,
        itemBuilder: (context, index) {
          return buildFactureCard(context, invoices[index]);
        },
      );
    }
  }
  ...
  Future<void> deleteFacture(int idfacture) async {
    try {
      ...
      MotionToast.success(
        width: 300,
        height: 50,
        title: Text("Invoice deleted successfully"),
        description: Text(""),
      ).show(context);
    } catch (e) {
      MotionToast.error(
        width: 300,
        height: 50,
        title: Text("failed to delete Invoice"),
        description: Text(""),
      ).show(context);
    }
  }
